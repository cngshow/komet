...
class ConceptChronologyTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @rest_concept_chronology = Concept.new(uuid: TEST_UUID, params: nil, action: ACTION_CHRONOLOGY, action_constants: ACTION_CONSTANTS).get_rest_class(json).send(:from_json, json)
  end
