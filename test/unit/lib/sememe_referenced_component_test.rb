...
class SememeReferencedComponentTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @rest_sememe_comp = Sememe.new(uuid: TEST_UUID, params: nil, action: ACTION_BY_REFERENCED_COMPONENT, action_constants: ACTION_CONSTANTS).get_rest_class.send(:from_json, json.first)
  end
