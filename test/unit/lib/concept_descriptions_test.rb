...
class ConceptDescriptionsTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @rest_concept_descriptions = Concept.new(uuid: TEST_UUID, params: nil, body_params: nil, action: ACTION_DESCRIPTIONS, action_constants: ACTION_CONSTANTS).get_rest_class(json.first).send(:from_json, json.first)
  end
