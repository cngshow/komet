...
class LogicGraphVersionTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @rest_graph_version = LogicGraph.new(uuid: TEST_UUID, params: nil, action: ACTION_VERSION, action_constants: ACTION_CONSTANTS).get_rest_class(json).send(:from_json, json)
  end
